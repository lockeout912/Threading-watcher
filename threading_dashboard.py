...
/* Chip color variants */
.chip-bull { border-color: rgba(52,255,154,.55) !important; background: rgba(52,255,154,.10) !important; color: #34ff9a !important; }
.chip-bear { border-color: rgba(255,91,110,.55) !important; background: rgba(255,91,110,.10) !important; color: #ff5b6e !important; }
.chip-neutral { border-color: rgba(255,204,102,.55) !important; background: rgba(255,204,102,.10) !important; color: #ffcc66 !important; }

.chip-open { border-color: rgba(52,255,154,.45) !important; background: rgba(52,255,154,.07) !important; color: rgba(255,255,255,.92) !important; }
.chip-pre  { border-color: rgba(88,215,255,.45) !important; background: rgba(88,215,255,.08) !important; color: rgba(255,255,255,.92) !important; }
.chip-after{ border-color: rgba(255,204,102,.45) !important; background: rgba(255,204,102,.08) !important; color: rgba(255,255,255,.92) !important; }
.chip-closed{ border-color: rgba(255,91,110,.45) !important; background: rgba(255,91,110,.08) !important; color: rgba(255,255,255,.92) !important; }
.chip-247  { border-color: rgba(185,147,255,.45) !important; background: rgba(185,147,255,.08) !important; color: rgba(255,255,255,.92) !important; }

.chip-trend{ border-color: rgba(185,147,255,.55) !important; background: rgba(185,147,255,.10) !important; color: #c9b3ff !important; }
.chip-range{ border-color: rgba(88,215,255,.55) !important; background: rgba(88,215,255,.10) !important; color: #58d7ff !important; }
.chip-mixed{ border-color: rgba(255,204,102,.55) !important; background: rgba(255,204,102,.10) !important; color: #ffcc66 !important; }

.chip-score{ border-color: rgba(255,255,255,.18) !important; background: rgba(255,255,255,.05) !important; color: rgba(255,255,255,.92) !important; }
.chip-mode{ border-color: rgba(255,255,255,.18) !important; background: rgba(255,255,255,.05) !important; color: rgba(255,255,255,.92) !important; }
...
def inject_autorefresh(seconds: int):
    # FIX: no external package, no warning text, just refresh.
    components.html(
        f"""
        <script>
        setTimeout(function() {{
...
<div class="marquee-wrap">
  <div class="marquee marq-{cls}">
    {html.escape(text)} &nbsp; • &nbsp; {html.escape(text)} &nbsp; • &nbsp; {html.escape(text)}
  </div>
</div>
""",
        unsafe_allow_html=True,
...
def chip(text: str, klass: str) -> str:
    return f'<div class="k_chip {klass}">{html.escape(text)}</div>'


def klass_bias(bias: str) -> str:
    b = (bias or "").upper()
    if "BULL" in b:
        return "chip-bull"
    if "BEAR" in b:
...
def klass_status(status: str) -> str:
    s = (status or "").upper()
    if "MARKET OPEN" in s:
        return "chip-open"
    if "PRE" in s:
...
def klass_regime(regime: str) -> str:
    r = (regime or "").upper()
    if "TREND" in r:
        return "chip-trend"
    if "RANGE" in r:
...
def compute_engine(df_1m: pd.DataFrame, df_5m: pd.DataFrame, mode: str) -> EngineOut:
    ...
    score = 0
    score += 32 if bias in ("BULLISH", "BEARISH") else 12
...
chips_html = "\n".join([
    chip(f"{out.bias} — {out.direction}", klass_bias(out.bias)),
    chip(mkt, klass_status(mkt)),
    chip(f"REGIME: {out.regime}", klass_regime(out.regime)),
    chip(f"SCORE: {out.score}/100", "chip-score"),
    chip(f"MODE: {mode}", "chip-mode"),
])

# IMPORTANT: Render the card ONLY via st.markdown with unsafe HTML.
# No st.write / st.text / st.code anywhere for this HTML string.
st.markdown(
    f"""
<div class="cc-card">
  <div class="k_subhead">{html.escape(subhead)}</div>

  <div class="k_price" style="color: {action_color};">{html.escape(fmt_price(out.price))}</div>
  <div class="k_action" style="color: {action_color};">{html.escape(out.action)}</div>
  ...
  <div class="k_small" style="margin-top:6px;">
    <span style="color:#34ff9a;"><b>LIKELY {html.escape(fmt_price(out.likely))}</b></span>
    &nbsp; | &nbsp;
    <span style="color:rgba(255,255,255,.92);"><b>POSS {html.escape(fmt_price(out.poss))}</b></span>
    &nbsp; | &nbsp;
    <span style="color:#34ff9a;"><b>STRETCH {html.escape(fmt_price(out.stretch))}</b></span>
  </div>

  <div class="k_small" style="margin-top:10px;">
    Invalid: <b>{html.escape(fmt_price(out.invalid))}</b> • Last update: <b>{html.escape(out.last_time)}</b>
  </div>
  ...
  <div class="kpi-row">
    <div class="kpi">
      ...
      <div class="value">{html.escape(out.bias)}</div>
      <div class="delta">Direction: {html.escape(out.direction)}</div>
    </div>
    <div class="kpi">
      <div class="label">VWAP (5m)</div>
      <div class="value">{html.escape(fmt_price(out.vwap_5m))}</div>
      <div class="delta">Anchor level for reclaim/reject</div>
    </div>
    <div class="kpi">
      <div class="label">ATR (5m)</div>
      <div class="value">{html.escape(fmt_price(out.atr_5m))}</div>
